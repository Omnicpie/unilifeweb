...
$soc = setupServer();
$pair = sendAES($soc);
$page = "groupfeed.php?groupid=".$_GET['groupid'];
    ...
    ob_start();
	sendtoServer($soc, $pair, "getgroupposts", $_GET['groupid'], $user);
    ...
    foreach($posts->children() as $post){
      ...
      Print '<div align="center" class="dateinfo" id="likes">'.$post['likes'].' Likes</div>';
			if($post['public_post'] == "1"){
                ...
                Print '<br/><div class="deletediv"><button style="background: none;border: none;color: var(--other-accent);cursor:pointer;" onclick="window.location.href=&quot;like.php?userid='.$user.'&amp;postid='.$post['id'].'&amp;group=1&amp;cur='.urlencode($page).'&quot">'.$likebutt.'</div> <div class="deletediv">';
                print'<i style="cursor:pointer;" onclick="comments('.$post['id'].', '.$_GET['groupid'].', 1)" class="fa fa-comment"></i>';
                print'</div></div>';
        Print '</div><div style="height:0;border: 1px solid var(--lighter-head);margin-bottom:20pt;border-top:0" class="comments" id="'.$post['id'].' '.$_GET['groupid'].'" ></div>';
    }
}
    ob_end_flush();
