	...
	<body style="display:flex;flex-direction:column;">
        ...
        <div class="prefs" style="display:flex;flex-grow:2;flex-direction:column">
            <div class="panels" style="display: flex;flex-grow: 2;">
                ...
                <?php
                    print '<h2 style="width: calc(100% - 33pt);float: left;margin: 5pt 0;">Chats</h2><button id="add" onclick="selectChat(event, 0)"><i style="pointer-events:none;" class="fa fa-pencil-square-o"></i></button><hr style="color:var(--lighter-head);width:100%;float:left;"/>';
                    ...
                    if ($chatXML->count()== 0 || (boolean) $chatXML == false){
                        ...
                        foreach($chatXML->children() as $chatchild){
                            Print'<button onclick="selectChat(event, &quot;'.$chatchild['id'].'&quot;)" id="chat">'.$chatchild['chat_name'].'</button>';
                        }
                    }
                    ?>
                </div>
                <div class="rightpanel" style="width:80%;">
                    <?php print'<iframe id="iframe" src="chat.php?chatid='.$_GET['chat'].'" frameborder="0"  style="width:100%;height:100%;" sandbox="allow-top-navigation allow-same-origin allow-scripts allow-popups allow-forms"></iframe>';?>
                </div>
            </div>
        </div>
